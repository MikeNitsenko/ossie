...
from .expressions import has_top_level_operator, qualify_bare_columns
...
def convert_cube_view_to_ossie(files, view, source=None, strict_fanout=True):
    ...
    selected, paths, required_cubes, joins = _resolve_view(
        selected_view, view, cubes)
    ...
    issues = _publication_issues(
        collection_issues, conversion_issues, selected, required_cubes)
    if strict_fanout:
...
def _resolve_view(view, view_name, cubes):
    ...
    for index, raw_entry in enumerate(entries):
        if not isinstance(raw_entry, dict):
            raise ConversionError(
                f"view '{view_name}': cubes[{index}] must be a mapping")
        ...
        if not isinstance(join_path, str) or not join_path.strip():
            raise ConversionError(
                f"view '{view_name}': cubes[{index}] has no non-empty join_path")
        parts = join_path.split(".")
        if any(not part for part in parts):
            raise ConversionError(
                f"view '{view_name}': invalid join_path '{join_path}'")
        if len(set(parts)) != len(parts):
            raise ConversionError(
                f"view '{view_name}': cyclic join_path '{join_path}' is not "
                "publishable")
...
def _find_join(cube, own_name, target):
    matches = [join for join in _as_named_list(
        cube.get("joins"), f"cube '{own_name}' joins")
    ...
    if len(matches) > 1:
        raise ConversionError(
            f"cube '{own_name}' declares more than one join to '{target}'")
    return matches[0]
...
def _members_from_entry(view_name, entry, cube_name, cube):
    ...
    if not isinstance(prefix, bool):
        raise ConversionError(
            f"view '{view_name}': prefix for cube '{cube_name}' must be boolean")
    alias = entry.get("alias") or cube_name
    if not isinstance(alias, str):
        raise ConversionError(
            f"view '{view_name}': alias for cube '{cube_name}' must be a string")

    collections = {
        kind: {member.get("name"): member for member in _as_named_list(
            cube.get(key), f"cube '{cube_name}' {key}") if member.get("name")}
        for kind, key in (
    ...
    if includes == "*":
        requested = [(name, name, {})
                     for members in collections.values() for name in members]
        ...
        for include in includes:
            if isinstance(include, str):
                ...
                if not isinstance(source_name, str) or not source_name:
                    raise ConversionError(
                        f"view '{view_name}': include for cube '{cube_name}' "
                        "must name a member")
                if not isinstance(output_name, str):
                    raise ConversionError(
                        f"view '{view_name}': alias for member '{source_name}' "
                        "must be a string")
                override = include
            else:
                raise ConversionError(
            ...
            if "." in output_name or "." in source_name:
                raise ConversionError(
                    f"view '{view_name}': paths are not allowed in includes "
                    f"('{source_name}')")
            requested.append((source_name, output_name, override))
    else:
        raise ConversionError(
            f"view '{view_name}': includes for cube '{cube_name}' must be '*' "
            "or a list")

    excludes = entry.get("excludes") or []
    if not isinstance(excludes, list) or not all(isinstance(v, str) for v in excludes):
        raise ConversionError(
            f"view '{view_name}': excludes for cube '{cube_name}' must be a list "
            "of member names")
    out = []
    for source_name, output_name, override in requested:
        found = [(kind, members[source_name]) for kind, members in collections.items()
        ...
        if kind == "dimension" and (dtype in ("geo", "switch")
                                    or definition.get("sub_query")):
        ...
        out.append(_SelectedMember(
            cube_name, source_name, final_name, kind, override))
    return out
...
def _resolve_source(view_name, requested, paths, required):
    ...
    if requested not in required:
        raise ConversionError(
            f"view '{view_name}': source '{requested}' is not in the projected "
            f"datasets {required}")
    return requested


def _project_cubes(cubes, required, joins, selected):
    needed_measures = _measure_closure(cubes, {
        (m.cube, m.source_name) for m in selected if m.kind == "measure"})
    projected = {}
    for cube_name in required:
        cube = copy.deepcopy(cubes[cube_name])
        cube["joins"] = list((joins.get(cube_name) or {}).values())
        cube["measures"] = [measure for measure in _as_named_list(
            cube.get("measures"), f"cube '{cube_name}' measures")
...
def _measure_closure(cubes, initial):
    ...
    while queue:
        ...
        texts.extend(f.get("sql") for f in measure.get("filters") or []
        ...
        for text in texts:
            ...
            for match in _CUBE_REF_RE.finditer(text):
                ...
                target = (cube_name, body) if not dot else (
                    cube_name if head in ("CUBE", "TABLE") else head, rest)
...
class _DimensionResolver:
    ...
    def expression(self, cube_name, member_name, qualified=False, stack=()):
        ...
        if dtype in ("geo", "switch") or dim.get("sub_query"):
            raise ConversionError(
                f"dimension '{cube_name}.{member_name}' has no safe static "
        ...
        if dim.get("case") is not None:
            ...
            expr = self._translate(
                dim["sql"], cube_name, qualified, stack + (marker,))
    ...
    def _translate(self, sql, cube_name, qualified, stack):
        ...
        out, _ = cube_sql_to_ossie(
            prepared,
            cube_name,
            resolve_ref=lambda body: self._resolve(
                body, cube_name, qualified, stack),
    ...
    def _case_expression(self, cube_name, member_name, case, qualified, stack):
        if not isinstance(case, dict):
            raise ConversionError(
                f"dimension '{cube_name}.{member_name}' has a non-mapping case")
        parts = []
        for branch in case.get("when") or []:
            if not isinstance(branch, dict) or branch.get("sql") is None:
                raise ConversionError(
                    f"dimension '{cube_name}.{member_name}' has an invalid "
                    "case.when entry")
            condition = self._translate(
                branch["sql"], cube_name, qualified, stack)
            parts.append(
                f"WHEN {condition} THEN "
                f"{self._case_label(branch, cube_name, qualified, stack)}")
        if not parts:
            raise ConversionError(
        ...
        if isinstance(otherwise, dict) and "label" in otherwise:
            parts.append(
                f"ELSE {self._case_label(otherwise, cube_name, qualified, stack)}")
...
def _apply_projection(model, cubes, selected, dim_resolver):
    ...
    pk_by_cube = {name: _primary_key_of(cube, name)
                  for name, cube in cubes.items()}
    resolver_issues = IssueLog()
    measure_resolver = _MeasureResolver(cubes, pk_by_cube, resolver_issues)
    converts = {key: measure_resolver.inlined(*key) is not None
                for key in measure_resolver.measures()}
    counts = {}
    ...
    for member in selected:
        if member.kind == "dimension":
            fields = {field["name"]: field
                      for field in datasets[member.cube].get("fields") or []}
            ...
            item["expression"]["dialects"][0]["expression"] = (
                dim_resolver.expression(member.cube, member.source_name))
            ...
            item["expression"]["dialects"][0]["expression"] = (
                _inline_metric_dimensions(expression, dim_resolver))
            _apply_override(item, member)
...
def _drop_cube_extension(item):
    extensions = [extension for extension in item.get("custom_extensions") or []
