...
import pytest
from _util import by_name, expr_of, model_of
...
def test_lossless_import_remains_unprojected():
    ...
    model = model_of(out)
    datasets = by_name(model["datasets"])
    assert {"id", "user_id", "status", "gross", "discount", "net_amount"} \
        <= set(by_name(datasets["orders"]["fields"]))
    assert {"count", "revenue", "average_value", "lifetime_value"} \
        <= set(by_name(model["metrics"]))
...
def test_hidden_computed_dimensions_are_inlined_in_fields_and_metrics():
    out, _, _ = _project()
    model = model_of(out)
    customer_name = by_name(by_name(model["datasets"])["users"]["fields"])[
        "customer_name"]
...
def test_wildcard_and_excludes_follow_cube_view_semantics():
    ...
    assert set(by_name(by_name(model["datasets"])["users"]["fields"])) == {
        "customer_full_name", "customer_city"}
    assert set(by_name(model["metrics"])) == {"average_value"}
...
def test_projection_converts_to_databricks_metric_view_without_member_collisions():
    databricks_src = pathlib.Path(__file__).resolve().parents[2] / "databricks" / "src"
    sys.path.insert(0, str(databricks_src))
    ...
    ossie, source, _ = _project()
    metric_view = load_databricks_yaml(
        convert_ossie_to_metric_view(ossie, source=source))
    ...
    assert [measure["name"] for measure in metric_view["measures"]] == [
        "average_value"]
